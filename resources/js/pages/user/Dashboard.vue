...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-8 p-6">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 flex items-center gap-2 text-muted-foreground">
                            ...
                            <span v-if="!emailVerified" class="text-warning ml-2 flex items-center gap-1 text-sm">
                                <AlertCircle class="h-4 w-4" />
                                (Email no verificado)
                            </span>
                            <span v-else class="text-success ml-2 flex items-center gap-1 text-sm">
            ...
            <div class="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div class="group animate-slide-up overflow-hidden rounded-xl border border-border bg-card shadow-lg transition-all duration-500 hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl"
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <div class="mt-2 flex items-center gap-1 text-xs" :class="remainingDownloads > 2
                                    ? 'text-success'
                                    : 'text-warning'
                                    ">
                                    <TrendingUp class="h-3 w-3" />
                                    <span>{{ remainingDownloads }} restantes
                                        hoy</span>
                                </div>
                            </div>
                            <div
                    ...
                    <div class="h-1 bg-gradient-to-r from-primary to-primary/60"
                        :style="{ width: `${(downloadsToday / 5) * 100}%` }"></div>
                </div>
                ...
                <div class="group animate-slide-up overflow-hidden rounded-xl border border-border bg-card shadow-lg transition-all duration-500 hover:-translate-y-1 hover:border-secondary/30 hover:shadow-xl"
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <div v-if="upcomingDueLoans.length > 0"
                                    class="text-warning mt-2 flex items-center gap-1 text-xs">
                                    <AlertCircle class="h-3 w-3" />
                                    <span>{{ upcomingDueLoans.length }} por
                                        vencer</span>
                                </div>
                                <div v-else class="text-success mt-2 flex items-center gap-1 text-xs">
                    ...
                    <div class="h-1 bg-gradient-to-r from-secondary to-secondary/60" :style="{
                ...
                <div class="group animate-slide-up overflow-hidden rounded-xl border border-border bg-card shadow-lg transition-all duration-500 hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl"
                    ...
                    <div class="h-1 bg-gradient-to-r from-primary to-primary/60" :style="{
                        width: `${(activeLoans.length / $page.props.auth.user.max_concurrent_loans) * 100}%`,
                    }"></div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                ...
                <div class="animate-slide-up rounded-xl border border-border bg-card p-6 shadow-lg"
                    ...
                    <div v-if="recentDownloads.length > 0" class="space-y-4">
                        <div v-for="download in recentDownloads.slice(0, 5)" :key="download.id"
                            ...
                            <div class="flex flex-1 items-center gap-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="truncate text-sm text-muted-foreground" v-if="download.book.authors">
                                        {{ download.book.authors }}
                                    </p>
                    ...
                    <div v-if="recentDownloads.length > 0" class="mt-4 border-t border-border pt-4">
                        <a href="/downloads"
                ...
                <div class="animate-slide-up rounded-xl border border-border bg-card p-6 shadow-lg"
                    ...
                    <div v-if="activeLoans.length > 0" class="space-y-4">
                        <div v-for="loan in activeLoans" :key="loan.id"
                            ...
                            <div class="flex flex-1 items-center gap-4">
                                <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-secondary/10 transition-transform duration-300 group-hover:scale-110"
                                    :class="{
                                        'bg-warning/10': upcomingDueLoans.some(
                                            (l) => l.id === loan.id,
                                        ),
                                    }">
                                    <BookMarked class="h-5 w-5 text-secondary" :class="{
                                        'text-warning':
                                            upcomingDueLoans.some(
                                                (l) => l.id === loan.id,
                                            ),
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="truncate text-sm text-muted-foreground"
                                        v-if="loan.physical_copy.book.authors">
                                        {{ loan.physical_copy.book.authors }}
                                    </p>
                                    <div class="mt-1 flex items-center gap-2">
                                        <Calendar class="h-3 w-3 text-muted-foreground" />
                                        <p class="text-xs" :class="upcomingDueLoans.some(
                                            (l) => l.id === loan.id,
                                        )
                    ...
                    <div v-if="activeLoans.length > 0" class="mt-4 border-t border-border pt-4">
                        <a href="/loans"
            ...
            <!-- Quick Actions -->
            <div class="animate-slide-up rounded-xl border border-border bg-card p-6 shadow-lg"
                :class="{ 'animate-in': isVisible }" style="animation-delay: 0.6s">
