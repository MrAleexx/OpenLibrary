...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="space-y-8 p-6">
            ...
            <!-- Empty State -->
            <div v-else class="flex flex-col items-center justify-center space-y-6 py-20 text-center">
                <div class="flex h-24 w-24 items-center justify-center rounded-full bg-muted/50 shadow-inner">
                ...
                <Link href="/books"
                    class="inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-3 text-primary-foreground shadow-lg transition-all hover:bg-primary/90 hover:shadow-primary/20">
            ...
            <div v-if="hasBooks && lastPage > 1" class="flex items-center justify-center gap-2 pt-8">
                <!-- Previous Button -->
                <Link v-if="books.links[0].url" :href="books.links[0].url" preserve-scroll
                ...
                <div class="flex items-center gap-1">
                    <template v-for="(page, index) in paginationRange" :key="index">
                        <span v-if="page === '...'" class="px-4 py-2 text-muted-foreground">
                            ...
                        </span>
                        <Link v-else-if="page !== currentPage" :href="`/books?page=${page}`" preserve-scroll
                ...
                <!-- Next Button -->
                <Link v-if="books.links[books.links.length - 1].url" :href="books.links[books.links.length - 1].url!"
                    preserve-scroll
                    class="inline-flex items-center gap-2 rounded-lg border border-border bg-background px-4 py-2 transition-colors hover:bg-muted">
